...
}requestAnimationFrame(drawMeter)};requestAnimationFrame(drawMeter)}function circular(analyser){var canvas=document.getElementById(Myself.canvasId),width=canvas.width,height=canvas.height,ctx=canvas.getContext("2d");var draw=function(){var array=new Uint8Array(128);analyser.getByteFrequencyData(array);ctx.clearRect(0,0,width,height);for(var i=0;i<(array.length);i++){var value=array[i];ctx.beginPath();ctx.arc(width/2,height/2,value*0.8,0,400,false);ctx.lineWidth=2;ctx.strokeStyle=(1,colorRgb(Myself.color,value/700));ctx.stroke();ctx.closePath();Myself.event({eventType:"energy",describe:value})}requestAnimationFrame(draw)};requestAnimationFrame(draw)}};